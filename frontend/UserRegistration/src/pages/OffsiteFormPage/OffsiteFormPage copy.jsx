...
const OffsiteFormPage = () => {
  ...
  const [offsiteLocations, setOffsiteLocations] = useState([]);
  ...
  const handleProceedToPay = async () => {
    ...
    try {
      ...
      const totalAmount = dependentsToPayFor.length * 1500;
  ...
  const validateFields = () => {
    ...
    const selectedLocationString = selectedLocation
      ...
      : "";
    const selectedCenterString = selectedCenter
  ...
  const fetchBookingDetails = async (bookingId) => {
    try {
      ...
      if (bookingDataResponse.offsiteLocation?.id) {
        setSelectedLocation(bookingDataResponse.offsiteLocation.id);
        localStorage.setItem(
          "selectedLocation",
          bookingDataResponse.offsiteLocation.id
        );
      } else {
        setSelectedLocation("");
      ...
      if (bookingDataResponse.diagnosticCenter?.id) {
        setSelectedCenter(bookingDataResponse.diagnosticCenter.id);
        localStorage.setItem(
          "selectedCenter",
          bookingDataResponse.diagnosticCenter.id
        );
      } else {
        setSelectedCenter("");
      ...
      if (bookingDataResponse.offsiteLocation?.id) {
        fetchNearestCenters(bookingDataResponse.offsiteLocation.id);
      }
  ...
  const fetchNearestLocations = async (userLat, userLng) => {
    try {
      const response = await api.get("/offsite-locations");
      const locations = response.data;

      const locationsWithMissingCoords = locations.filter(
        (location) => !location.latitude || !location.longitude
      );

      if (locationsWithMissingCoords.length > 0) {
        setCoordinatesError(
          "Some locations are missing latitude or longitude data."
        );
      }
      ...
      setOffsiteLocations(nearbyLocations);
    } catch (error) {
      console.error("Error fetching offsite locations:", error);
      alert("Failed to load offsite locations. Please try again.");
    }
  };

  const fetchNearestCenters = async (locationId) => {
    ...
    try {
      const response = await api.get(
        `/diagnostic-centers/location/${locationId}`
      );
  ...
  useEffect(() => {
    if (selectedLocation) {
      fetchNearestCenters(selectedLocation);
    } else {
  ...
  const handleNextClick = async () => {
    ...
    try {
      ...
      const offsiteCollectionData = {
        userId: userId,
        offsiteLocationId: Number(selectedLocation),
      ...
      return {
        ...
        userDetails: {
          ...
          dependents,
          offsiteLocation: selectedLocationObject,
  ...
  const saveReportDataToLocalStorage = (bookingResult) => {
    const reportData = {
      ...
      offsiteLocation: bookingResult?.userDetails.offsiteLocation,
            ...
            <div className={styles.middleSide}>
              ...
              <div className={styles.formGroup}>
                ...
                <select
                  ...
                  disabled={!selectedLocation || diagnosticCenters.length === 0}
                >
              ...
              <div className={styles.actionButtons}>
                ...
                {requiresPayment && (
                  <PaymentModal
                    ...
                    pricePerDependent={1500}
                    basePrice={0}
                    onProceedToPay={handleProceedToPay}
                    totalAmount={
                      (!initialBookingId
                        ? dependents.length
                        : newDependents.length) * 1500
                    }
                    dependentsDetails={
                      !initialBookingId ? dependents : newDependents
                    }
