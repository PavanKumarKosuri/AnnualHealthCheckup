// superAdmin/src/components/SuperAdmin/Options/Reports/GetHrs.jsx
...
const GetHRs = () => {
  ...
  const fetchHrReports = async () => {
    try {
      const response = await api.get(`/api/hr/get-all-hrdata`, {
        params: filters,
      });
  ...
  const handleConfirmDelete = async () => {
    try {
      await api.delete(`/api/hr/delete-hr/${deleteId}`);
      fetchHrReports();
      setShowModal(false);
    } catch (error) {
  ...
  const saveEdit = async () => {
    try {
      ...
      await api.put(`/api/hr/update-hr/${currentEdit.id}`, updatedHr);
  ...
  const saveAdd = async () => {
    try {
      ...
      await api.post(`/api/hr/add-hr`, newEntryHr);
      setErrorMessage("");
